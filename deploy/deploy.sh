...
WEB_DIR="$HOME/sepia-cuaderno/web"
...
start() {
  ...
  docker run -d --name "$NAME" --restart unless-stopped -p "$PORT:80" "${NGINX_ARGS[@]}" "$IMAGE" >/dev/null
}
...
serving() {  # the landing at / and the app at /hoy, up to ~20 s
  for _ in $(seq 1 20); do
    sleep 1
    if page_has / '/landing/landing.css' && page_has /hoy 'id="root"'; then return 0; fi
  done
  return 1
}
...
echo "nginx no sirve la landing (/) o la app (/hoy) en :$PORT. Últimas líneas del registro:" >&2
...
if [ -d "$WEB_DIR/previous" ]; then
  ...
  start
  if serving; then echo "La versión anterior vuelve a estar publicada." >&2; else echo "La versión anterior tampoco responde." >&2; fi
