...
import { courseLabel, formatAverage, NOTE_KIND_LABEL, plural, shortDate, TERM_LABEL } from '../../lib/format';
...
export function studentSummary(f: StudentFile, term: number): string {
  ...
  for (const sc of f.courses) {
    ...
    const avg = cell?.final ?? cell?.average;
    const total = sc.absences + sc.justified;
    const parts = [
      avg == null ? `sin notas en la ${TERM_LABEL[term]}` : `media ${formatAverage(avg)} en la ${TERM_LABEL[term]}`,
      total ? `${plural(total, 'falta', 'faltas')} (${plural(sc.justified, 'justificada', 'justificadas')})` : 'sin faltas',
