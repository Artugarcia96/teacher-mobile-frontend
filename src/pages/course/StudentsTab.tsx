...
function StudentLine({ s }: { s: StudentRow }) {
  ...
  const sub = (reason || measures.length || flag || absences) ? (
    <span className="students__sub">
      ...
      {absences && <span className="students__abs">{plural(s.absences, 'falta', 'faltas')}</span>}
    </span>
  ) : undefined;
...
export default function StudentsTab({ course }: { course: CourseDetail }) {
  ...
  if (isLoading) body = <SkeletonList rows={8} />;
    ...
    body = (
      <Section title={plural(data.length, 'alumno', 'alumnos')} footer={`Nota: media de la ${TERM_LABEL[term]}.`} className="students">
        <List>
          {data.map((s) => <StudentLine key={s.id} s={s} />)}
          <Row lead={<UserPlus size={18} className="students__add" />} title={<span className="students__add">Añadir alumnos</span>}
