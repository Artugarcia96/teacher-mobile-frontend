...
import { pageCaption } from '../../features/papers/pageLabels';
...
function ReviewStudent({ review: r, activityId, courseId, onNext, exit }: {
  ...
  const info = r.pages; // aligned with pages_urls: exam pages in order, extra sheets after
              ...
              <div className="review-thumbs">
                {pages.map((src, i) => (
                  <button key={src} type="button" onClick={() => pageRefs.current[i]?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
                    ...
                    <span className="num">{extraTag(i) ? 'extra' : i + 1}</span>
                  </button>
                ))}
              </div>
            )}
            <div className="review-scroll">
              {pages.map((src, i) => (
                <div key={src} ref={(el) => { pageRefs.current[i] = el; }} className={`review-page${zoom === i ? ' review-page--zoom' : ''}`}>
                  ...
                  <img src={src} alt={extraTag(i) ?? `Página ${i + 1}`} loading={i < 2 ? 'eager' : 'lazy'}
