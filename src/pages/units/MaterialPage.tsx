...
import {
  ...
  useRewriteBlock, useUpdateMaterial, type MaterialDetail, type ReviewNote,
} from '../../api/units';
...
import {
  AIBadge, Button, Callout, Dot, EmptyState, IconButton, List, Menu, Page, Progress, Row, Section, Segmented, SkeletonList, Spinner,
  Switch, useFeedback, type MenuItem,
} from '../../ui';
import { figureCaption } from './blocks';
import DocView, { docOutline, elementLabel } from './DocView';
...
/** Material generated by Sepia: the ContentDoc on paper (apuntes, ficha, resumen, lectura sencilla) or as slides, with its
...
function Material({ m, courseId, unitPath }: { m: MaterialDetail; courseId: string; unitPath: string }) {
  ...
  // The PDF is what students get (never with solutions); the solucionario is the teacher's.
  ...
  return (
    <Page title={title} eyebrow={eyebrow} {...page} wide={slides}
      ...
      <div className={`material-body${slides ? '' : ' material-body--reading'}`}>
        ...
        {doc && <Problems m={m} />}
        {doc && isDraft(m) && <ReviewNotes notes={m.review ?? []} doc={doc} />}
...
/** What the verification withdrew or could not fix, while the material is a draft: one row each, saying what and why
 *  (never dropped silently). A problem left unfixed leads to its element. */
function ReviewNotes({ notes, doc }: { notes: ReviewNote[]; doc: ContentDoc }) {
  if (!notes.length) return null;
  const go = (id: string) => document.querySelector(`[data-element="${CSS.escape(id)}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  return (
