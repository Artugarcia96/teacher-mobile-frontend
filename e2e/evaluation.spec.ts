...
// No AI call is made, and the draft it accepts is put back afterwards: the demo keeps its drafts for the next run.
// (Every other Evaluación flow is in e2e/flows/evaluacion-*.spec.ts.)
test('review report comments: accept a draft, then a grade that no longer matches its comment', async ({ page, request }, info) => {
  const errors = trackErrors(page);
  const login = await (await request.post(`${API}/api/auth/login`, { data: { email: 'demo@sepia.es', password: 'sepia1234' } })).json();
  const headers = { authorization: `Bearer ${login.access_token}` };
  const courses: { id: string; label: string }[] = await (await request.get(`${API}/api/courses`, { headers })).json();
  const course = courses.find((c) => /2\.º ESO B/.test(c.label))!;
  const rows = async () => (await (await request.get(`${API}/api/courses/${course.id}/evaluation/1`, { headers })).json()).rows as
    { student: { id: string }; comment_source: string | null; comment_status: string | null }[];
  ...
  try {
    ...
    for (const r of await rows()) {
      if (drafts.has(r.student.id) && r.comment_status === 'final') {
        await request.put(`${API}/api/courses/${course.id}/evaluation/1/students/${r.student.id}`, { headers, data: { comment_status: 'draft' } });
      }
    }
  }
});
